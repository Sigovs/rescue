...
async function loadPartials() {
  ...
  await Promise.all(
    [...slots].map(async (slot) => {
      ...
      try {
        const res = await fetch(`partials4/${name}.html`, { cache: "no-cache" });
...
/* ---------- 9. SUBPAGE NAV REWRITE ----------
   On a subpage (car.html etc.) the shared header/footer use same-page
   anchors (#section) that don't exist here — repoint them at the home
   page so nav + logo + CTA travel back to index4.html#section. */
function linkifyToHome() {
  if (!document.body.hasAttribute("data-subpage")) return;
  const home = "index4.html";
  document
    .querySelectorAll('.site-header a[href^="#"], .site-footer a[href^="#"]')
    .forEach((a) => {
      const h = a.getAttribute("href");
      if (h === "#") return;                 // leave bare-# placeholders (social) alone
      a.setAttribute("href", h === "#top" ? home : home + h);
    });
}
...
async function boot() {
  ...
  linkifyToHome();      // subpage: repoint shared nav anchors to home
